      ...
      async fetchServices(req: Request,res: Response) {
       ...
       async postNSSPassword(req: Request,res: Response) {
         try {
            ...
            if(newp != rnewp) return res.status(204).json({ message: `new password mismatch` })

            const user = await Auth.withCredential(nss_no,oldp)
            if(!user) return res.status(204).json({ message: `old password doesnt exist` })
            const resp = await Auth.updateSSOPassword(nss_no, { password: hashPassword(rnewp) })
            if(resp){
              res.status(200).json(resp)
            } else {
              res.status(204).json({ message: `no records found` })
