    ...
    async fetchForms(req: Request, res: Response) {
        try {
            ...
            if (options.length) {
                ...
                // Nothing to evaluate yet — either the evaluation window
                // hasn't opened for this semester, or this student has no
                // registered/assessed courses in the active session at all.
                ...
                res.status(200).json({ status: inWindow ? 'not started' : 'not registered', data: options });
            }
        } catch (error: any) {
            console.log(error);
