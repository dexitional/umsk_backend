...
class HrsController {
    ...
    postNSSRegister(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            try {
                const password = (0, password_1.hashPassword)(req.body.password);
    ...
    postNSSPassword(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (newp != rnewp)
                    return res.status(204).json({ message: `new password mismatch` });
                const user = yield Auth.withCredential(nss_no, oldp);
                if (!user)
                    return res.status(204).json({ message: `old password doesnt exist` });
                const resp = yield Auth.updateSSOPassword(nss_no, { password: (0, password_1.hashPassword)(rnewp) });
                if (resp) {
                    res.status(200).json(resp);
                }
                else {
