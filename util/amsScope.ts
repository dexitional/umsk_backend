...
function heldRoles(roles: Role[] = [], modulePrefix: string): string[] {
  return roles
    .filter((r) => r?.module === modulePrefix && r?.role)
    .map((r) => r.role as string);
}

// 'UG' | 'PG' | null (null = holds no role for this module at all).
export function categoryScope(roles: Role[], modulePrefix: string): 'UG' | 'PG' | null {
  const tags = heldRoles(roles, modulePrefix);
  if (tags.some((t) => t.endsWith('-pg'))) return 'PG';
  if (tags.some((t) => t.endsWith('-ug'))) return 'UG';
  return null;
}
...
export function categoryWhere(scope: 'UG' | 'PG' | null): any {
  if (scope === 'PG') return { categoryId: 'PG' };
  if (scope === 'UG') return { categoryId: { not: 'PG' } };
  return { categoryId: '__no_category_role__' };
}

export function hasRoleTag(roles: Role[] = [], tag: string): boolean {
  return roles.some((r) => r?.role === tag);
}
...
export function categoryScopeAcross(roles: Role[], modulePrefixes: string[]): 'UG' | 'PG' | null {
